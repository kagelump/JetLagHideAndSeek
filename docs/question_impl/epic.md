...
|---|---|---|---|
| **Measuring** | "I'm [Distance] from my nearest ___. Are you closer to or farther from yours?" | Closer / Farther | Circle centered on the seeker's target POI, radius = seeker's distance to it |
...
| **Tentacles** | "Of all the ___ within ___ of me, which are you closest to?" | A named POI | Voronoi cell of the named POI, clipped to the seeker's radius circle |

### How these modes are actually used (product intent)

- **Measuring is a planning tool.** The seeker compares *their own* nearest POI
  to the hider's nearest POI of the same category. The intended phrasing is:
  *"I am 1.2 km from my nearest movie theater — are you closer or farther from
  yours?"* The seeker picks the POI/location first; the distance is a derived
...
- **Thermometer is a planning tool for halving the remaining hiding space.**
  Seekers want to plan *how far to travel, from where to where, and which
  question it spends* so the perpendicular bisector cuts the remaining area in
  roughly half. The travel distance is usually chosen *after* the two endpoints
...
exist. Geometry tasks especially: the failing assertions *are* the spec.
...
- **Distance math**: use `haversineDistanceMeters(lat1, lon1, lat2, lon2)` from
  ...
  it is *not* a dependency. Installed turf packages are only `@turf/circle`,
...
question. Measuring/Tentacles categories are *not* `MatchingCategory` values, so
