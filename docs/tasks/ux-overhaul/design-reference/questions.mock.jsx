...
const QTYPES = [
    {
        id: "radar",
        title: "Radar",
        detail: "Preview a distance from a movable map pin.",
        cost: "Draw 2, pick 1",
        time: "5 min",
    },
    {
    ...
    return (
        <div>
            <SheetHeader
                ...
                onBack={p.onBack}
                accessory={
                    <LockBtn locked={p.locked} onClick={p.onToggleLock} />
                }
            />
            ...
            <ChipGroup
                options={["500m", "1km", "2km", "5km", "10km", "Other"]}
                value={p.distance}
                onChange={p.setDistance}
            />
            <p
                style={{
                    fontSize: "var(--text-meta)",
            ...
            >
                📍 35.6878, 139.7239 · long-press the map to move the pin
            </p>
        </div>
    );
}
    ...
    return (
        <div>
            ...
            >
                ...
                >
                    Move, then compare hotter or colder.
                </span>
                <QuestionMeta cost="Draw 2, pick 1" time="5 min" />
            </div>
            ...
            <SegmentedControl
                options={["Start", "End"]}
                value={p.activePin === "start" ? "Start" : "End"}
                onChange={(v) => p.setActivePin(v.toLowerCase())}
                fullWidth
            />
            <p
                style={{
                ...
                {window.MAP_POIS.map((poi, i) => {
                    ...
                    return (
                        <ListRow
                            key={poi.id}
                            title={poi.name}
                            description={`${[0.4, 0.9, 1.1, 1.6][i]} km away`}
                            active={on}
                            leading={
                                <span
                                    style={{
                                        ...
                                        height: 12,
                                        borderRadius: 999,
                                        border: `2px solid ${on ? "var(--teal)" : "var(--border-default)"}`,
                                        background: on
                                            ? "var(--teal)"
                                            : "transparent",
    ...
    return (
        <div>
            ...
            >
                ...
                >
                    Compare distance to a place or boundary.
                </span>
                <QuestionMeta cost="Draw 3, pick 1" time="5 min" />
            </div>
            <FieldLabel>Compared to</FieldLabel>
            <ListRow
                title="Nearest rail station"
                description="Shinjuku Station · 1.4 km"
                trailing={
                    <span
    ...
    return (
        <div>
            ...
            >
                ...
                >
                    Same ward as the hider?
                </span>
                <QuestionMeta cost="Draw 2, pick 1" time="5 min" />
            </div>
            <FieldLabel>Category</FieldLabel>
            <ListRow
                title="Same ward (admin division)"
                trailing={
                    <span
                        style={{ color: "var(--text-secondary)", fontSize: 22 }}
                    >
