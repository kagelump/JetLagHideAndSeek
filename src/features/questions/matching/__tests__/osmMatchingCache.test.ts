...
// Disable the bundled POI path so findMatchingFeaturesWithCache tests always
// exercise the mocked Overpass path.
...
// Mock spatial index so findMatchingFeaturesWithIndex tests control the path.
const mockQuerySpatial = jest.fn();

jest.mock("../spatialIndex", () => ({
    clearSpatialIndexCache: jest.fn(),
    querySpatialIndex: (...args: unknown[]) => mockQuerySpatial(...args),
}));
...
describe("containsSearchCircle", () => {
    ...
    it("returns true when overscan circle contains exact-radius request", () => {
        expect(
            containsSearchCircle(35.68, 139.76, 75_000, 35.68, 139.815, 50_000),
        ).toBe(true);
    });
});
...
describe("findMatchingFeaturesWithCache", () => {
    ...
    it("fetches again when center is outside the cached overscan circle", async () => {
        const farCenter: [number, number] = [141.0, 35.68];
...
describe("findMatchingFeaturesWithIndex", () => {
    beforeEach(async () => {
        jest.clearAllMocks();
        mockQuerySpatial.mockReset();
        await resetAll();
    });

    it("falls through to Overpass when bundle has no covering region", async () => {
        // Default mock: regionCoveringPoint returns null → spatial index skipped.
        mockFetchAndParse.mockResolvedValue(hospitalFeatures);
        ...
        expect(result.source).toBe("network");
        expect(mockQuerySpatial).not.toHaveBeenCalled();
...
describe("clearOsmMatchingCache", () => {
    ...
    it("clears memory caches and spatial index", async () => {
        mockFetchAndParse.mockResolvedValue(hospitalFeatures);
        await findMatchingFeaturesWithCache("hospital", tokyoCenter);
        jest.clearAllMocks();

        const removed = await clearOsmMatchingCache();
        expect(removed).toBeGreaterThan(0);

        // Memory should be cleared — next call hits network.
        const result = await findMatchingFeaturesWithCache(
            "hospital",
            tokyoCenter,
        );
    ...
    it("leaves non-OSM-matching keys intact", async () => {
        await AsyncStorage.setItem("other-key", "value");
        mockFetchAndParse.mockResolvedValue(hospitalFeatures);
        await findMatchingFeaturesWithCache("hospital", tokyoCenter);

        await clearOsmMatchingCache();

        const other = await AsyncStorage.getItem("other-key");
