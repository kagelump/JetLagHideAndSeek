...
- `assets/`: committed runtime data — `default-zones/`, `poi/`, `measuring/`,
  `map/`.
- `tools/data-viewer/`: local web viewer for committed data assets (zones,
  POIs, measuring lines, transit bundles) — `node tools/data-viewer/server.mjs`
  serves it; `build.mjs` emits the static variant. Use it to eyeball
  pipeline output before committing regenerated bundles.
