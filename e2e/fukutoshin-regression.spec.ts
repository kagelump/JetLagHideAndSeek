...
test("C13: Fukutoshin Line auto-detect from Higashi-shinjuku picks relation, not way", async ({
    ...
    // Install Overpass mocks
    await mockOverpass(page, [
        overpassRoute("fukutoshin-station-discovery.json", [
            "[railway=station]",
            "[railway=stop]",
        ]),
        overpassRoute("fukutoshin-transit-graph.json", ["rel(bn)"]),
    ]);
    ...
    // Assert the station preview resolved from the cached transit graph
    // (no per-question exact-line Overpass call needed)
    await expect(page.getByText("Stations matched: 16")).toBeVisible({
        timeout: 15000,
    });
});
