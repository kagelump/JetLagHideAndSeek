...
describe("normalizeName", () => {
  ...
  it("strips suffix even with leading space", () => {
    assert.equal(normalizeName("Shinjuku Station", ["Station"]), "shinjuku");
  });
...
describe("dedupeOsmStations", () => {
  it("deduplicates by id (region-boundary overlap)", () => {
    const records = [
      { id: "osm:node:1", name: "A", lat: 35, lon: 139, normalizedName: "a" },
      { id: "osm:node:1", name: "A", lat: 35, lon: 139, normalizedName: "a" },
    ];
  ...
  it("deduplicates by wikidata, keeping most complete", () => {
    const records = [
      { id: "osm:node:1", name: "A", lat: 35, lon: 139, normalizedName: "a", wikidata: "Q1" },
      { id: "osm:node:2", name: "A+", lat: 35, lon: 139, normalizedName: "a", wikidata: "Q1", nameEn: "A" },
    ];
  ...
  it("merges same normalized name within 150 m", () => {
    const records = [
      { id: "osm:node:1", name: "Shinjuku", lat: 35.689, lon: 139.700, normalizedName: "shinjuku" },
      { id: "osm:node:2", name: "Shinjuku Station", lat: 35.690, lon: 139.701, normalizedName: "shinjuku" },
    ];
    const { kept, stats } = dedupeOsmStations(records);
    assert.equal(kept.length, 1);
  ...
  it("keeps both when same name but far apart", () => {
    const records = [
      { id: "osm:node:1", name: "Park", lat: 35.0, lon: 139.0, normalizedName: "park" },
      { id: "osm:node:2", name: "Park", lat: 36.0, lon: 140.0, normalizedName: "park" },
    ];
    const { kept } = dedupeOsmStations(records);
