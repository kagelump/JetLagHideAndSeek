...
export async function buildMeasuringArtifact({
    ...
    for (const catDef of categories) {
        ...
        const seqPath = join(tmpDir, `${catDef.key}.seq`);
        console.log(`  [measuring/${catDef.key}] Exporting to GeoJSONSeq...`);
        execFileSync(
            "osmium",
            [
                "export",
        ...
        // Stream GeoJSONSeq line by line.
        ...
        for await (const line of rl) {
            ...
            // Apply post-filter.
            if (!passesPostFilter(effectiveCatDef, feature.properties ?? {})) {
                continue;
            }
            ...
            for (const lf of lineFeatures) {
                if (catDef.geometry === "polygon-dissolve") {
                    if (
                        ...
                        for (const ls of lineStrings) {
                            const bbox2 = computeBbox(ls.geometry.coordinates);
                            features.push({
                                type: "Feature",
                                bbox: bbox2,
                ...
                for (const ls of lineStrings) {
                    const tolerance = SIMPLIFY_TOLERANCES[catDef.key] ?? 0.0001;
                    const simplified =
                        catDef.key === "high-speed-rail"
                            ? ls
                            : simplifyFeature(ls, tolerance);
                    const bbox2 = computeBbox(simplified.geometry.coordinates);
                    features.push({
                        type: "Feature",
                        bbox: bbox2,
        ...
        if (features.length === 0) {
            console.log(`  [measuring/${catDef.key}] No features — skipping`);
            continue;
        }
        ...
        if (isPolygonDissolve) {
            ...
            const tileDeg = overrides.dissolve?.tileDeg ?? DISSOLVE_TILE_DEG;
            const overlapDeg =
